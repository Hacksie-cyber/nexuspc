...
export function CatalogTab({ activeTab, products, showToast, setActiveTab }: CatalogTabProps) {
  ...
  const handleExportInventory = () => {
    const rows = [
      ...
      ...products.map(p => [
        ...
        p.name, p.category, p.brand, p.price.toString(), p.stock.toString(),
  ...
  const CompatibilityFields = ({ product, onChange }: { product: Partial<Product>; onChange: (p: Partial<Product>) => void }) => (
    <div className="border border-dashed border-green-200 bg-green-50/40 rounded-2xl p-5 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] text-green-500">— used by PC Builder</span>
      </div>
      {['cpu', 'motherboard'].includes(product.category || '') && (
        <div className="space-y-2">
          <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">
            CPU Socket <span className="text-gray-300 normal-case font-normal">(e.g. AM5, LGA1700)</span>
          </label>
          <input type="text" placeholder="e.g. AM5" value={product.socket || ''}
            onChange={e => onChange({ ...product, socket: e.target.value })}
      ...
      {['motherboard', 'ram'].includes(product.category || '') && (
        <div className="space-y-2">
          <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">
            RAM Type <span className="text-gray-300 normal-case font-normal">(e.g. DDR4, DDR5)</span>
          </label>
          <select value={product.ramType || ''} onChange={e => onChange({ ...product, ramType: e.target.value })}
            className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-green-500 transition-all">
      ...
      {['cpu', 'gpu', 'psu'].includes(product.category || '') && (
        <div className="space-y-2">
          <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">
            {product.category === 'psu' ? 'PSU Wattage (W)' : 'TDP / Power Draw (W)'}
            <span className="text-gray-300 normal-case font-normal ml-1">
              {product.category === 'psu' ? '(e.g. 650, 850)' : '(e.g. 125, 200)'}
            </span>
          </label>
          <input type="number" placeholder={product.category === 'psu' ? 'e.g. 650' : 'e.g. 125'}
            value={product.watts || ''}
            onChange={e => onChange({ ...product, watts: parseInt(e.target.value) || undefined })}
            className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-green-500 transition-all" />
        </div>
      )}
    </div>
    ...
    <>
                ...
                <form onSubmit={handleUpdateProduct} className="space-y-6">
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <input required type="text" value={editingProduct.brand}
                  ...
                  <div className="grid grid-cols-3 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <input required type="number" value={editingProduct.price || 0}
                        onChange={e => setEditingProduct({ ...editingProduct, price: parseInt(e.target.value) || 0 })}
                        className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-green-500 transition-all" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Stock</label>
                      <input required type="number" value={editingProduct.stock || 0}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-4">
                      <input required type="url" placeholder="https://example.com/image.jpg"
                        value={editingProduct.img}
                        onChange={e => setEditingProduct({ ...editingProduct, img: e.target.value })}
                        className="flex-1 bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-green-500 transition-all" />
                      {editingProduct.img && (
                        <div className="w-12 h-12 rounded-lg border border-gray-100 overflow-hidden bg-gray-50 shrink-0">
                          <img src={editingProduct.img} alt="Preview" className="w-full h-full object-cover"
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea rows={4} value={editingProduct.description}
                  ...
                  {['cpu', 'motherboard', 'ram', 'gpu', 'psu'].includes(editingProduct.category || '') && (
                    <CompatibilityFields product={editingProduct} onChange={p => setEditingProduct(p as Product)} />
                  )}
      ...
      <div className="p-8 flex-1">
                    ...
                    {filteredProducts.map(product => (
                      <tr key={product.id} className="hover:bg-gray-50/50 transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center text-xl border border-gray-200">{product.icon}</div>
                            <div className="font-bold text-sm text-gray-900">{product.name}</div>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <span className="px-2 py-1 bg-gray-100 text-gray-600 rounded-md text-[10px] font-bold uppercase tracking-wider">{product.category}</span>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <div className="w-16 h-1 bg-gray-100 rounded-full overflow-hidden">
                              <div className={`h-full rounded-full ${product.stock <= 5 ? 'bg-orange-500' : 'bg-green-500'}`}
          ...
          <motion.div key="inventory" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {[
                ...
                { label: 'Low Stock', value: products.filter(p => p.stock <= 5 && p.stock > 0).length, color: 'text-orange-500' },
                ...
                <div key={i} className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                  <div className="text-xs text-gray-400 uppercase tracking-widest font-bold mb-1">{s.label}</div>
                    ...
                    {products.map(product => (
                      <tr key={product.id} className="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="px-6 py-4 font-mono text-xs text-gray-400">
                          NPC-{product.category.toUpperCase().slice(0, 3)}-{product.id.toString().padStart(4, '0')}
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                          <button onClick={() => handleUpdateStock(product.id, product.stock + 10)}
        ...
        {activeTab === 'add-product' && (
          <motion.div key="add-product" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="max-w-2xl mx-auto">
            <div className="bg-white rounded-3xl border border-gray-200 p-8 shadow-xl">
              ...
              <form onSubmit={handleAddProduct} className="space-y-6">
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Brand</label>
                    <input required type="text" value={newProduct.brand}
                ...
                {['cpu', 'motherboard', 'ram', 'gpu', 'psu'].includes(newProduct.category || '') && (
                  <CompatibilityFields product={newProduct} onChange={setNewProduct} />
                )}
                <button type="submit"
